...
## sample log location
log_in = open(os.path.join(paninc_libPath,'bin','data','pan_incident_data_config.txt'),'r').readlines()
print log_in
...
try:
	urllib2.urlopen('https://'+splunkserver+':'+splunkport+'/services/receivers/simple?index='+index+'&sourcetype='+sourcetype, 'this is a test')
except:
	...
	while (True):
		...
		for l in log:
			...
			if x >= EPS:
				...
				time.sleep(5)
				continue;
			ts =  datetime.datetime.now().strftime('%b %d %H:%M:%S 1,%Y/%m/%d %H:%M:%S,')
			evt =  ts+l[38:].replace('[\r\n]','')
