##############
# modify these vars if neccessary
##############
...
## sample log location
#log_in = open(os.path.join(paninc_libPath,'bin','data','pan_incident.txt'),'r').readlines()
log_in = []
for filename in os.listdir(os.path.join(paninc_libPath,'bin','data','reports')):
	with open(os.path.join(paninc_libPath,'bin','data','reports',filename)) as f:
...
try:
	urllib2.urlopen('https://'+splunkserver+':'+splunkport+'/services/receivers/simple?index='+index+'&sourcetype=pan_log', 'this is a test')
except:
...
def replay():
  ...
  while (True):
    ...
    for l in log_in:
      ...
      urllib2.urlopen('https://'+splunkserver+':'+splunkport+'/services/receivers/simple?index='+index+'&sourcetype='+sourcetype, evt)
